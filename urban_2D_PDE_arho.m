...
figure()
hold on
r1=rectangle('Position',[tlist1(1),0,tlist1(end)-tlist1(1),0.55],'FaceColor',[0.8500, 0.3250, 0.0980],'FaceAlpha',0.1,'EdgeColor','k',...
    'LineWidth',0.1);
r2=rectangle('Position',[tlist2(1),0,tlist2(end)-tlist2(1),0.55],'FaceColor',[0.4940, 0.1840, 0.5560],'FaceAlpha',0.1,'EdgeColor','k',...
    'LineWidth',0.1);
r3=rectangle('Position',[tlist3(1),0,tlist3(end)-tlist3(1),0.55],'FaceColor',[0, 0.4470, 0.7410],'FaceAlpha',0.1,'EdgeColor','k',...
    'LineWidth',0.1);
...
ylim([0,0.55])
xlabel('t')
legend('Pre-suppression','Suppression','Post-suppression')
hold off
...
figure()
pdeplot(results3.Mesh,XYData=u3(:,6000),ColorBar='on')
colormap('jet')
xlim([a,b])
xlabel('x')
ylim([a,b])
...
nr = length(location.x); 
f = zeros(N,nr);
As=quantile(utail,0.5,'all');
us=griddata(X, Y, utail, location.x, location.y);
damp=kappa1*(us>As)+kappa2*(us<=As);
f(1,:) = -state.u(1,:)+damp.*state.u(1,:).*state.u(2,:)+damp.*alpha;
f(2,:) = -state.u(1,:).*state.u(2,:)-state.u(3,:).*state.u(2,:)+beta;
end
