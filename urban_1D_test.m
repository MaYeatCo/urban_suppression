...
%We solve it via loop
for j=1:length(eps)
%P0=[0.1*(eps(j)>=0)+1.5*(eps(j)<0)*sqrt(-eps(j)*D11/D13)];
%The initial magnitude is defined so that we close enough to a steady state
P0=[0.1*(eps(j)<0)+1.5*(eps(j)>=0)*sqrt(-eps(j)*D11/D13)];

%PDE part
param1=[Du, Dv, Dw, alpha, beta,chi,eps(j)];
...
%recording
UsimEnd(j)=rms(u1(end,:)'-ubar);
UampEnd(j)=UweakS(end);
end

%plotting RMS, you need to run thsi code multiple times if you want to
%compare different initial conditions. Save those rms and add to your plot
%appropriately
figure()
hold on
